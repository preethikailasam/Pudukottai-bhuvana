...
class Gateway extends RTEC_Payment {
	...
	public function pp_redirect( $settings = array() ) {
		...
		$rtec       = RTEC();
		$args       = array(
			'fields' => array( 'event_id', 'venue', 'confirmation_code', 'action_key', 'email', 'first', 'last', 'phone' ),
			'where'  => array(
		...
		$payment->description = Helper::get_description( $event_name, $entry_id );
		...
		// Amount.
		$payment->set_total_amount( new TaxedMoney( $amount, $currency ) );
