...
class Extension extends AbstractPluginIntegration {
	...
	public function __construct() {
		parent::__construct(
			array(
				'name' => __( 'Registrations for The Events Calendar', 'knit-pay' ),
			)
		);
	...
	public function setup() {
		...
		add_action( 'rtec_payment_cancel_after_post', array( $payment, 'payment_process_cancellation' ) );
		add_action( 'rtec_payment_success_before_post', array( $payment, 'payment_success_add_notice' ) );
		add_action( 'rtec_payment_cancel_before_post', array( $payment, 'payment_cancel_add_notice' ) );
		add_action( 'rtec_payment_test_before_post', array( $payment, 'payment_test_add_notice' ) );

		add_action( 'rtec_payment_listeners', array( $this, 'rtec_payment_listeners' ) );
		add_filter( 'rtec_currency_code', array( $this, 'rtec_currency_code' ) );
		add_filter( 'rtec_after_payment_table', array( $this, 'knit_pay_payment_button' ) );
		add_filter( 'rtec_the_submission_status', array( $this, 'rtec_check_if_doing_payments' ), 11, 3 );
	...
	public function source_text( $text, Payment $payment ) {
		$text = __( 'Registrations for The Events Calendar', 'knit-pay' ) . '<br />';

		$text .= sprintf(
			'<a href="%s">%s</a>',
			get_edit_post_link( $payment->source_id ),
			/* translators: %s: source id */
			sprintf( __( 'Invoice %s', 'knit-pay' ), $payment->source_id )
		);
	...
	public function source_description( $description, Payment $payment ) {
		return __( 'Registrations for The Events Calendar', 'pronamic_ideal' );
	}
