...
class Gateway extends EM_Gateway {
	...
	function em_my_bookings_booking_actions( $message, $EM_Booking ) {
		...
		if ( $this->uses_gateway( $EM_Booking ) && $EM_Booking->booking_status == $this->status ) {
			if ( empty( $EM_Booking->booking_meta['knitpay_payment_url'] ) ) {
				$this->booking_form_feedback( array( 'result' => true ), $EM_Booking );
			}
	 ...
	 */
	function booking_form_feedback( $return = array(), $EM_Booking = false ) {
	...
	private function start( EM_Booking $EM_Booking, $return ) {
		...
		$payment->description = Helper::get_description( $payment_method, $EM_Booking->booking_id );
		...
		// Amount.
		$payment->set_total_amount( new TaxedMoney( $EM_Booking->get_price(), $currency ) );
	...
	function booking_form_feedback_fallback( $feedback ) {
		...
		$return = $this->booking_form_feedback( array( 'result' => true ), $EM_Booking );

		if ( $return['result'] ) {
			$feedback .= '<br />' . __( 'To finalize your booking, please click the following button to make the payment.', 'knit-pay' ) . $this->get_pay_button( $return['redirect_url'] );
	...
	function mysettings() {
		...
		<tbody>
		  <?php em_options_input_text( esc_html__( 'Success Message', 'em-pro' ), 'em_' . $this->gateway . '_booking_feedback', esc_html__( 'The message that is shown to a user when a booking is successful whilst being redirected for payment.', 'em-pro' ), __( 'Please wait whilst you are redirected to proceed with payment.', 'knit-pay' ) ); ?>
		  <?php em_options_input_text( esc_html__( 'Success Free Message', 'em-pro' ), 'em_' . $this->gateway . '_booking_feedback_free', esc_html__( 'If some cases if you allow a free ticket (e.g. pay at gate) as well as paid tickets, this message will be shown and the user will not be redirected.', 'em-pro' ), __( 'Booking successful.', 'events-manager' ) ); ?>
		  <?php em_options_input_text( esc_html__( 'Thank You Message', 'em-pro' ), 'em_' . $this->gateway . '_booking_feedback_completed', esc_html__( 'If you choose to return users to the default Events Manager thank you page after a user has paid, you can customize the thank you message here.', 'em-pro' ), __( 'Thank you for your payment. Your transaction has been completed, and a receipt for your purchase has been emailed to you along with a separate email containing account details to access your booking information on this site.', 'knit-pay' ) ); ?>
		  <?php em_options_select( esc_html__( 'Configuration', 'knit-pay' ), 'em_' . $this->gateway . '_config_id', Plugin::get_config_select_options( $this->gateway ) ); // TODO default value ?>
		  <?php em_options_input_text( esc_html__( 'Payment Description', 'knit-pay' ), 'em_' . $this->gateway . '_payment_description', sprintf( __( 'Available tags: %s', 'knit-pay' ), sprintf( '<code>%s</code>', '{booking_id}' ) ), __( 'Events Manager Pro {booking_id}', 'knit-pay' ) ); ?>
		</tbody>
		</table>
		<?php
	}
