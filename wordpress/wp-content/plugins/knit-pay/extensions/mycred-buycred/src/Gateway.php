<?php
namespace KnitPay\Extensions\MyCred;
...
class Gateway extends myCRED_Payment_Gateway {
	 ...
	 * Construct
	 */
	public function __construct( $gateway_prefs = array() ) {
		...
		parent::__construct(
			array(
				...
				'gateway_logo_url' => '',
				'defaults'         => array(
					'title'               => PaymentMethods::get_name( 'knit_pay', __( 'Knit Pay', 'pronamic_ideal' ) ),
		...
		add_filter( 'mycred_buycred_sort_gateways', array( $this, 'mycred_buycred_sort_gateways' ) );
	}
	...
	public function prep_sale( $new_transaction = false ) {
		...
		$payment->description = Helper::get_description( $this->prefs, $this );
		...
		// Amount.
		$payment->set_total_amount( new TaxedMoney( $this->cost, $currency ) );
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id( 'title' ); ?>"><?php _e( 'Title', 'knit-pay' ); ?></label>
		...
		<div class="form-group">
			<label for="<?php echo $this->field_id( 'payment_description' ); ?>"><?php _e( 'Payment Description', 'knit-pay' ); ?></label>
			...
			<small><?php echo sprintf( __( 'Available tags: %s', 'knit-pay' ), sprintf( '<code>%s</code>', '{transaction_id}, {point_count}' ) ); ?></small>
		</div>
		<div class="form-group">
			<label for="<?php echo $this->field_id( 'config_id' ); ?>"><?php _e( 'Configuration', 'knit-pay' ); ?></label>
