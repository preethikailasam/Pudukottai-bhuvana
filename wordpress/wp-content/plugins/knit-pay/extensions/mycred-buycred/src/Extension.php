...
class Extension extends AbstractPluginIntegration {
	...
	public function __construct() {
		parent::__construct(
			array(
				'name' => __( 'myCRED - buyCRED', 'knit-pay' ),
			)
		);
	...
	public function setup() {
		...
		add_action( 'plugins_loaded', array( $this, 'init_gateway' ), 10000 );
	}
	...
	public function init_gateway() {
		...
		add_filter( 'mycred_setup_gateways', array( $this, 'mycred_setup_gateways' ) );
		add_filter( 'mycred_dropdown_currencies', array( $this, 'mycred_dropdown_currencies' ) );
	}
	...
	public function mycred_setup_gateways( $installed ) {
		...
		$installed['knit_pay'] = array(
			'title'         => 'Knit Pay',
			'documentation' => '', // TODO add documentation URL
			'callback'      => array( __NAMESPACE__ . '\Gateway' ),
	...
	public static function redirect_url( $url, $payment ) {
		...
		$gateway   = buycred_gateway( $payment->get_method() );
	...
	public function source_text( $text, Payment $payment ) {
		$text = __( 'buyCRED', 'pronamic_ideal' ) . '<br />';

		if ( ! function_exists( 'buycred_get_pending_payment_id' ) ) {
			$text .= sprintf( __( 'Transaction %s', 'pronamic_ideal' ), $payment->source_id );
			return $text;
		}
	...
	public function source_description( $description, Payment $payment ) {
		return __( 'buyCRED Transaction', 'pronamic_ideal' );
	}
