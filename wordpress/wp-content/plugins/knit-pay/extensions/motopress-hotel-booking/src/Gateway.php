...
class Gateway extends HotelBooking_Gateway {
	...
	public function __construct( $title, $id ) {
		...
		add_filter( 'mphb_gateway_has_sandbox', array( $this, 'hide' ), 10, 2 );
		add_filter( 'mphb_gateway_has_instructions', array( $this, 'hide' ), 10, 2 );
	...
	public function processPayment( Booking $booking, HotelBooking_Payment $hb_payment ) {
		...
		$payment->description = Helper::get_description( $this->payment_description, $booking, $hb_payment );
		...
		// Amount.
		$payment->set_total_amount( new TaxedMoney( $hb_payment->getAmount(), $currency ) );

		// Method.
		$payment->method = $this->id;
		...
		$groupFields = array(
			...
			Fields\FieldFactory::create(
				...
				array(
					...
					'description' => sprintf( __( 'Available tags: %s', 'knit-pay' ), sprintf( '<code>%s</code>', '{booking_id}, {payment_id}' ) ),
					'default'     => __( 'MotoPress Hotel Booking {booking_id}', 'knit-pay' ),
				)
			),
		);
