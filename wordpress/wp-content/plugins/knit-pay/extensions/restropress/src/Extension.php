...
class Extension extends AbstractPluginIntegration {
	...
	public function __construct() {
		parent::__construct(
			array(
				'name' => __( 'Restro Press', 'pronamic_ideal' ),
			)
		);
	...
	public static function redirect_url( $url, $payment ) {
		...
		switch ( $payment->get_status() ) {
			...
			case Core_Statuses::EXPIRED:
				...
				rpress_send_back_to_checkout( '?payment-mode=' . $payment->method );
				break;
	...
	public static function status_update( Payment $payment ) {
		...
		rpress_insert_payment_note( $payment_id, sprintf( __( 'Knit Pay Transaction ID: %s', 'knit-pay' ), $payment->get_transaction_id() ) );
	...
	public function source_text( $text, Payment $payment ) {
		$text = __( 'Restro Press', 'knit-pay' ) . '<br />';
	...
	public function source_description( $description, Payment $payment ) {
		return __( 'Restro Press Order', 'knit-pay' );
	}
