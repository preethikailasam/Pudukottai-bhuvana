...
class Gateway extends Stripe_Gateway {
	...
	protected function create_session_data( $payment ) {
		...
		$session_data['payment_intent_data']['application_fee_amount'] = $this->get_application_fee_amount( $this->get_payment_amount( $payment ) );

		return $session_data;
	}

	private function get_application_fee_amount( $amount ) {
