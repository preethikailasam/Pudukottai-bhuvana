...
class Integration extends AbstractGatewayIntegration {
	...
	public function get_settings_fields() {
		...
		$fields[] = array(
			...
			'title'    => __( 'Enabled Payment Methods', 'knit-pay' ),
			'type'     => 'description',
			'callback' => array( $this, 'field_enabled_payment_methods' ),
		);
	...
	public function get_config( $post_id ) {
		...
		if ( empty( $config->enabled_payment_methods ) ) {
			$config->enabled_payment_methods = array( PaymentMethods::CREDIT_CARD );
		}
	...
	public function field_enabled_payment_methods( $field ) {
		...
		$supported_payment_methods_ids = $gateway->get_supported_payment_methods();
		$supported_payment_methods_ids = array_diff( $supported_payment_methods_ids, array( 'stripe' ) );
		...
		foreach ( $supported_payment_methods_ids as $payment_method ) {
			...
			$supported_payment_methods[ $payment_method ] = $name;
		}
	...
	public function save_post( $post_id ) {
		...
		if ( empty( $post['_pronamic_gateway_stripe_enabled_payment_methods'] ) ) {
			$post['_pronamic_gateway_stripe_enabled_payment_methods'] = array( PaymentMethods::CREDIT_CARD );
		}
